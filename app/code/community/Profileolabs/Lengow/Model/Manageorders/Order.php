...
class Profileolabs_Lengow_Model_Manageorders_Order extends Varien_Object
{
	...
	*/
    ...
    {
    	$order = Mage::getModel('sales/order')->getCollection()->addAttributeToFilter('order_id_lengow',$idLengow)->addAttributeToSelect('order_id_lengow');
    	if(sizeof($order) > 0)
    		return true;
    	else
    		return false;
    ...
    */
    ...
    {
		$query = "SELECT DISTINCT(`main_table`.tax_calculation_rate_id), rate FROM `tax_calculation` AS `main_table` LEFT JOIN tax_calculation_rate  ON main_table.tax_calculation_rate_id = tax_calculation_rate.tax_calculation_rate_id";

		$resource = Mage::getSingleton('core/resource');
		$con = $resource->getConnection('core_read');

		$taxes = $con->fetchAll($query);
		unset($resource, $query, $con);

		$_taxes = array();
		foreach($taxes as $t)
			$_taxes[$t['tax_calculation_rate_id']] = $t['rate'];

		return $_taxes;
    }

	/**
	 * Update order values to manage taxes
	 * Used for products
	 * BETA
	 */
	protected function _customTotals()
	{
    	$_items = $this->_getQuote()->getAllItems();

    	foreach($_items as $_item)
    	{
    		$tax_class_id = $_item->getData('tax_class_id');
    		$tax_percent = (float) $_item->getData('tax_percent');
    		$item_price = (float) $_item->getData('custom_price');
    		$item_qty = (int) $_item->getData('qty');

    		if($tax_percent == 0 && isset($this->_taxes[$tax_class_id]))
    		{
				$_item->setData('tax_percent', $this->_taxes[$tax_class_id]);
				$tax_percent = (float) $this->_taxes[$tax_class_id];        			
    		}

    		$tax_amount = ($item_price * $item_qty) / (($tax_percent / 100) + 1);
    		$tax_amount = -($tax_amount - ($item_price * $item_qty));
    		$_item->setData('tax_amount', $tax_amount);
    		$_item->setData('base_tax_amount', $tax_amount);

    		$item_tax_amount = $item_price / (($tax_percent / 100) + 1);
    		$item_tax_amount = -($tax_amount - $item_price);
    		$item_ht = $item_price - $item_tax_amount;

    		// $_item->setData('calculation_price', $item_ht);
    		// $_item->setData('base_calculation_price', $item_ht);
    		$_item->setData('custom_price', $item_ht);
    		$_item->setData('base_custom_price', $item_ht);
    		$_item->setData('price', $item_ht);
    		$_item->setData('base_price', $item_ht);
    		
    		$_item->setData('price_incl_tax', $item_price);
    		$_item->setData('base_price_incl_tax', $item_price);
    		
    		$row_total = ($item_price * $item_qty) - $tax_amount;
    		$_item->setData('row_total', $row_total);
    		$_item->setData('base_row_total', $row_total);
    	}
    	unset($_item, $_items, $tax_class_id, $tax_percent, $item_price, $item_qty, $item_ht, $item_tax_amount, $tax_amount);
	}
	 ...
	 * BETA
	 */
	protected function _customOrderTotals($order)
	{
    	$tax_class_id = $this->_getQuote()->getAllItems();
    	$tax_class_id = $tax_class_id[0]->getTaxClassId();

		$tax_percent = (float) $this->_taxes[$tax_class_id];        			

    	$order_total = (float) $order->getData('grand_total');
    	$order_products = (float) $order->getData('subtotal');
		$shipping_amount = (float) $order->getData('shipping_amount');

		$tax_amount = $order_total / (($tax_percent / 100) + 1);
		$tax_amount = -($tax_amount - $order_total);

		$order->setData('tax_amount', $tax_amount);
		$order->setData('base_tax_amount', $tax_amount);

		unset($tax_class_id, $tax_percent, $order_total, $order_products, $shipping_amount, $tax_amount);
		return $order;
	}
	...
	protected function _addProductsToQuote(array $orderLw)
	{
		...
		foreach($productsToIterate as $key=>$productLw)
		{
			...
			if($this->getConfig()->getAny('lengow_mo', 'manageorders', 'force_tax'))
        		$this->_customTotals();
	...
	protected function _saveOrder(array $orderLw, $quoteId)
	{
		...
		$additionalData = array("from_lengow"=>$orderLw['IdFlux'],
