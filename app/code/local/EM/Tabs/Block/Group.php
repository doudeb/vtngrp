	...
	*/
	public function getTitleTab($number = 1){
		...
		if($this->getData('instance')){
			...
			$titleTab = isset($params['title_'.$number][$storeId]) && !empty($params['title_'.$number][$storeId]) ? $params['title_'.$number][$storeId] : $params['title_'.$number][0];
		}
		else if($this->getData('title_'.$number)){
			$titleArray = unserialize(base64_decode($this->getData('title_'.$number)));
			$titleTab = isset($titleArray[$storeId]) && !empty($titleArray[$storeId]) ? $titleArray[$storeId] : $titleArray[0];
		}	
		return $titleTab;
	}
