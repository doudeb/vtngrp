...
Mage::Log('[EMC] Discount amount found'.$amount);
    ...
    if($moduleConfig['order_mode'] == 'automatique' && strpos($data['shipping_method'], "envoimoinscher") !== false)
    {
      ...
      if($payment['method'] != 'checkmo')
      {
        ...
        // order informations
        $quotInfo = array("collecte" => $helper->setCollectDate((int)$moduleConfig['pickup_date']), 
          "delai" => "aucun", "code_contenu" => $moduleConfig["content"],
          "url_tracking" => Mage::getUrl('envoimoinscher/tracking/update', array('code' => $data['protect_code'], 'order_id' => $data['entity_id'])),
          "operateur" => $shipInfo['ope'], "service" => $shipInfo['srv'],  "valeur" => $data['subtotal'],
	      "assurance.selection" => "off", "description" => $helper->constructDesc($allItems),
          "version" => Mage::helper('envoimoinscher')->getModuleInfoToApi("version"), "module" => Mage::helper('envoimoinscher')->getModuleInfoToApi("name")
        );
