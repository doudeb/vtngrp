...
class Boxtale_Envoimoinscher_Model_Emc_Services extends Mage_Core_Model_Abstract 
{
  ...
  {
    return $this->getServices('zones_es != '.self::ZONE_INTERNATIONAL);
  } 
  ...
  {
    return $this->getServices('family_es = '.self::FAM_SMALL_PARCELS.' AND zones_es != '.self::ZONE_INTERNATIONAL);
  } 
  ...
  {
    return $this->getServices('family_es = '.self::FAM_EXPRESS.' AND zones_es != '.self::ZONE_INTERNATIONAL);
  } 
  ...
  public function listServices() 
  {
    ...
    foreach($collection->query() as $s => $service)
    {
      $arr[$service['id_es']] = array('id' => $service['id_es'], 'value' => $service['code_es'], 'label' => $service['label_es'],
      'desc' => $service['desc_es'], 'opeCode' => $service['emc_operators_code_eo'],
      'zones' => $service['zones_es'], 'pricing' => $service['price_type_es'], 'label_store' => $service['label_store_es'],
      'desc_store' => $service['desc_store_es'], 'operator' => $service['operator'], 'family' => $service['family_es']
      );
    }
    return $arr;
  }
  ...
  public function insertService($ope, $srv)
  {
    ...
    desc_es, desc_store_es, label_store_es, price_type_es, is_parcel_point_es, zones_es, family_es)
    VALUES
    (:code, :opeCode, :label, :desc, :descStore, :labelStore, :pricing, :point, :zones, :family)";
    $binds = array(
    'code'      => $srv['code'],
    'opeCode'     => $ope['code'],
    'label'     => $srv['label'],
    'desc'     => $srv["srvInfos"]['label_backoffice'],
    'descStore'     => $srv["srvInfos"]['label_store'],
    'labelStore'     => $srv['label'],
    'pricing'     => Boxtale_Envoimoinscher_Model_Emc_Services::PRICING_API,
    'point'     => (int)($srv["delivery"] == "DROPOFF_POINT"),
    'zones'   => (int)$srv["srvInfos"]['offer_zoning'],
    'family' => $srv["srvInfos"]["family_es"]);
